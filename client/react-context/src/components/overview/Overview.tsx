...
import { url_data, httpGET } from '../../utils/common'
...
function Overview() {
  ...
  const [percentRecovered, setPercentRecovered] = useState<Number>(0)
  const [percentDeath, setPercentDeath] = useState<Number>(0)
  const [percentActive, setPercentActive] = useState<Number>(0)
  ...
  const fetchCountryList = async (url: string) => {
    await fetch(url, httpGET)
      .then((response) => response.json())
      .then((data) => {
        setCountries((data.Countries.map((el: ICountry) => el.Country)).sort())
      })
      .catch(err => {
        console.log(err)
      })
  ...
  const fetchDataGlobal = async (url: string) => {
    setIsLoading(true)
  ...
  const fetchDataCountry = async (url: string, newValue: string | null) => {
    setIsLoading(true)
    ...
    await fetch(url, httpGET)
      ...
      .then((res) => {
        ...
        const obj = res.Countries.find((el: any) => el.Country === newValue)
    ...
    <div>
      {isLoading ? (
        <div className='main-overview'>
          ...
          <MainContext.Provider value={{ country }}>
            <div className='main-overview'>
